...
// Seed AJAX endpoint and CSRF token for actions
$token = Security::get_token();
$htmlHeadXtra[] = '<script>window.USER_PROFILE_AJAX_URL = "'.api_get_path(WEB_PLUGIN_PATH).'user_profile/ajax.php"; window.userProfileToken = "'.$token.'"; window.USER_PROFILE_COMPANY_LIST_URL = "'.api_get_path(WEB_PLUGIN_PATH).'user_profile/company_list.php";</script>';
$htmlHeadXtra[] = '<script src="'.api_get_path(WEB_PLUGIN_PATH).'user_profile/assets/js/common.js"></script>';
// Page-specific JS: handle delete company
$confirm = addslashes(get_plugin_lang('ConfirmYourChoice', 'UserProfilePlugin'));
$deletedLbl = addslashes(get_plugin_lang('Deleted', 'UserProfilePlugin'));
$notAllowed = addslashes(get_plugin_lang('NotAllowed', 'UserProfilePlugin'));
$htmlHeadXtra[] = '<script>(function(w,$){$(function(){
  $(document).on("click", ".js-delete-company", function(e){
    e.preventDefault();
    var $btn = $(this); var id = parseInt($btn.data("company"), 10) || 0;
    if (!id) { return; }
    if (!w.confirm("'.$confirm.'")) { return; }
    if (typeof postAction !== "function") { try { w.location.reload(); } catch(e){} return; }
    postAction("delete_company", { company_id: id }, function(resp){
      if (resp && resp.status === "ok") {
        try { var $card = $btn.closest(".card.company"); if ($card.length) { $card.fadeOut(200, function(){ $(this).remove(); }); } } catch(e){}
        try { if (document.location.search.indexOf("id=") !== -1) { w.location.href = (w.USER_PROFILE_COMPANY_LIST_URL || "company_list.php"); } } catch(e){}
        try { if (typeof showToast === "function") { showToast("'.$deletedLbl.'", "success"); } } catch(e){}
      } else if (resp && resp.status === "not_allowed") {
        try { if (typeof showToast === "function") { showToast("'.$notAllowed.'", "error"); } } catch(e){}
      }
    });
  });
});})(window, window.jQuery || window.$);</script>';
...
foreach ($rows as $row) {
    ...
    if ($canEdit) {
        ...
        $deleteLink = '<a href="#" class="js-delete-company" data-company="'.(int) $row['id'].'" title="'.get_lang('Delete').'">'.$deleteIcon.'</a>';

        $editIcon = Display::url(
            Display::return_icon('edit.png', get_lang('Edit'), [], ICON_SIZE_SMALL),
            $editUrl
        );
        // Right-side controls: delete + edit
        $rightControls = '<span class="ml-2">'.$deleteLink.'</span><span class="ml-2">'.$editIcon.'</span>';
    }
    echo '<div class="card-title d-flex justify-content-between align-items-center">'
        .'<span><strong>'.Security::remove_XSS($title).'</strong></span>'
        .$rightControls
        .'</div>';
