...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($check && isset($_POST['action'])) {
        ...
        if (!$isAuthorized) {
            ...
            case 'delete_company':
                // Stricter authorization: only platform admins, session admins or DRH
                if (!(api_is_platform_admin() || api_is_session_admin() || api_is_drh())) {
                    $status = 'not_allowed';
                    break;
                }
                $companyId = (int) ($_POST['company_id'] ?? 0);
                $debug['delete_company'] = ['company_id' => $companyId];
                if ($companyId > 0) {
                    $tblEntreprise = Database::get_main_table(UserProfilePlugin::TABLE_ENTREPRISE);
                    $tblUserCompany = Database::get_main_table(UserProfilePlugin::TABLE_USER_COMPANY);
                    $urlId = (int) api_get_current_access_url_id();
                    // Remove user -> company links first (no FK present by default)
                    try { Database::delete($tblUserCompany, ['company_id = ?' => $companyId]); } catch (Exception $e) {}
                    // Delete the company within the current portal
                    $deleted = false;
                    try {
                        $deleted = (bool) Database::delete($tblEntreprise, ['id = ? AND access_url_id = ?' => [$companyId, $urlId]]);
                    } catch (Exception $e) {
                        $debug['delete_company']['error'] = $e->getMessage();
                    }
                    $status = $deleted ? 'ok' : 'error';
                    $debug['delete_company']['deleted'] = (bool) $deleted;
                } else {
                    $status = 'error';
                    $debug['delete_company']['error'] = 'invalid_company_id';
                }
                break;
